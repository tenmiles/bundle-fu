class BundleFu
  ...
  def self.bundle_files(filenames)
    ...
    filenames.each do |filename|
      buffer << "/* --------- #{filename} --------- */\n"
      begin
        content = File.read(File.join(Rails.public_path, filename))
      rescue
  ...
  module InstanceMethods
    ...
    def bundle(options={}, &block)
      ...
      abs_filelist_paths = [:css, :js].inject({}) do | hash, filetype |
        ...
        hash[filetype] = File.join(Rails.public_path, paths[filetype], filename)
        hash
      end
      ...
      [:css, :js].each do |filetype|
        ...
        abs_path = File.join(Rails.public_path, output_filename)
        ...
        unless new_filelist == filelist
          FileUtils.mkdir_p(File.join(Rails.public_path, paths[filetype]))
